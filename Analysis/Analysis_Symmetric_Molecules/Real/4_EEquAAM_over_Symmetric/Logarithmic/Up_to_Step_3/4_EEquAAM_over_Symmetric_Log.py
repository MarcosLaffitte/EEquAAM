...
#    time (seconds) taken by the methods to analyze each reaction 5 times.     #
...
for eachReaction in list(graphsByMap.keys()):
    ...
    for i in range(repetitions):
        printProgress(lengthC*100/len(list(graphsByMap.keys())), lengthC, len(list(graphsByMap.keys())), i+1)
...
# save original data
originalData = [timesReactionAUX, timesReactionISO, timesReactionCGR]
outputFile = open("Linear_Times_Backup.pkl", "wb")
pickle.dump(originalData, outputFile)
outputFile.close()
...
plt.xticks(ticks = X, labels = ["Step 1", "Step 2", "Step 3"], fontsize = 9, rotation = 45)
plt.ylabel("Mean and Std of " + r"$Log_{10}$" + " of running time [s]\n" + "over " + str(repetitions) + " iterations", fontsize = 9)
...
plt.ylabel(r"$Log_{10}$" + " of running times [s]\n" + "over " + str(repetitions) + " iterations", fontsize = 10)
